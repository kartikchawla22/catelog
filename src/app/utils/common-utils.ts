...
export const AllLocations = catalogJSON.data.locations.map(obj => { return {locationName: obj.name, dealers_id: obj.dealers_id};});

export const AllBranches = catalogJSON.data.locations.reduce((acc, key) => acc.concat(key.branches),[]);

export const AllCategories = AllBranches.reduce((acc, key) => acc.concat(key.categories), []);

export const locationsAndBranches = catalogJSON.data.locations.map(obj => { return {location: obj.name, dealers_id: obj.dealers_id,
  branches: obj.branches.map(branch => {return {branchName: branch.name, branchID: branch.branch_id}})};});


export const getParticularBranch = (branchId: string) => {
 return AllBranches.find(branch => branch.branch_id === branchId);
};

export const getParticularCategory = (categoryName: string) => {
  return AllCategories.find(category => category.name.replace(/ /g, '') === categoryName.replace(/ /g, ''));
};

export const getBranchNameFromBranchId = (branchId: string) => {
  return AllBranches.find(branch => branch.branch_id === branchId).name;
};
